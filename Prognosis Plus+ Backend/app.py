...
tr.replace({'prognosis': {'Fungal infection': 0, 'Drug Reaction': 1, 'GERD': 2, 'Chronic cholestasis': 3, 'Allergy': 4,
                          'Peptic ulcer diseae': 5, 'AIDS': 6, 'Diabetes ': 7, 'Gastroenteritis': 8,
                          'Bronchial Asthma': 9, 'Hypertension ': 10,
                          'Migraine': 11, 'Cervical spondylosis': 12,
                          'Paralysis (brain hemorrhage)': 13, 'Jaundice': 14, 'Malaria': 15, 'Chicken pox': 16,
...
clf3 = VotingClassifier(estimators=[('DecisionTree', clf0), ('RandomForest', clf1), ('GradientBoosting', clf2)], voting='hard')
clf3 = clf3.fit(X, y)
...
def Home():
    ...
    return render_template("forms1.html", user_image = pic1)
...
def predict():
    ...
    if option1 == option2:
        return render_template("forms1.html",user_image = pic3)
